...
"alterF/Const" forall (f :: Maybe a -> Const b (Maybe a)) . alterF f = \k m -> Const . getConst . f $ lookup k m
 #-}
#if MIN_VERSION_base(4,8,0)
-- This makes it slower! Why is this broken? This stays just as broken if I
-- only rewrite for k ~ Int.
{-# RULES
"alterF/Identity" alterF = alterFIdentity
 #-}
#endif
#endif
...
alterFIdentity f k t = case go (coerce f) k t of
    AltSmaller t' -> Identity t'
    AltBigger t' -> Identity t'
    AltAdj t' -> Identity t'
    AltSame -> Identity t
  where
